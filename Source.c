// Codename: JeuVidéal
// YouTube:  https://www.youtube.com/channel/UCppt2Hb61BD2x5qQ1oA4m-g
// Abonne toi sinon je code un programme pour te détruire
...
int __stdcall WinMain(_In_ HINSTANCE Instance, _In_opt_ HINSTANCE PreviousInstance, _In_ PSTR CommandLine, _In_ INT CmdShow)
{
    ...
    {
        LogMessageA(LL_ERROR, "[%s] Couldn't find the NtQueryTimerResolution function in ntdll.dll! GetProcAdresss failed ! Error 0x%08lx!", __FUNCTION__, GetLastError());
    ...
    {
        LogMessageA(LL_WARNING, "[%s] Failed to set global timer resolution!", __FUNCTION__);
    ...
    {
        LogMessageA(LL_WARNING, "[%s] Failed to set process priority! Error 0x%08lx!", __FUNCTION__, GetLastError());
    ...
    if ((Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\FONTS\\6x7Font.bmpx", &g6x7Font)) != ERROR_SUCCESS)
    {
    ...
    if (InitializeSkeletor() != ERROR_SUCCESS)
    {
        LogMessageA(LL_ERROR, "[%s] Failed to initialize Skeletor!", __FUNCTION__);

        MessageBoxA(NULL, "Failed to initialize Skeletor!", "Error!", MB_ICONERROR| MB_OK);
...
DWORD CreateMainGameWindow(void)
{
    ...
    if (RegisterClassExA(&WindowClass) == 0)
    {
        ...
        LogMessageA(LL_ERROR, "[%s] RegistrationExA Failed! Error 0x%08lx!", __FUNCTION__, Result);
    ...
    gGameWindow = CreateWindowExA(0, WindowClass.lpszClassName, "Le JeuVidéal", WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, 640, 480, NULL, NULL, GetModuleHandleA(NULL), NULL);
...
void ProcessPlayerInput(void)
{
    ...
    int16_t LeftKeyIsDown = GetAsyncKeyState(VK_LEFT) | GetAsyncKeyState('Q');

    int16_t RightKeyIsDown = GetAsyncKeyState(VK_RIGHT) | GetAsyncKeyState('D');

    int16_t UpKeyIsDown = GetAsyncKeyState(VK_UP) | GetAsyncKeyState('Z');
    ...
    if (!gSkeletor.MovementRemaining)
    {
        if (DownKeyIsDown)
        {
            if (gSkeletor.ScreenPosY < GAME_RES_HEIGHT - 16)
            {
                gSkeletor.Direction = DOWN;

                gSkeletor.MovementRemaining = 16;
            }
        }
        else if (LeftKeyIsDown)
        {
            if (gSkeletor.ScreenPosX > 0)
            {
                gSkeletor.Direction = LEFT;
        ...
        {
            if (gSkeletor.ScreenPosX < GAME_RES_WIDTH - 16)
            {
                gSkeletor.Direction = RIGHT;
        ...
        {
            if (gSkeletor.ScreenPosY > 0)
            {
                gSkeletor.Direction = UP;
    ...
    {
        gSkeletor.MovementRemaining--;

        if (gSkeletor.Direction == DOWN)
            {
                gSkeletor.ScreenPosY++;
            }
            else if (gSkeletor.Direction == LEFT)
            {
                gSkeletor.ScreenPosX--;
            }
            else if (gSkeletor.Direction == RIGHT)
            {
                gSkeletor.ScreenPosX++;
            }
        else if (gSkeletor.Direction == UP)
            {
                gSkeletor.ScreenPosY--;
            }

        switch (gSkeletor.MovementRemaining)
            {
            case 16:
            {
                gSkeletor.SpriteIndex = 0;

                break;
            }
            case 12:
            {
                gSkeletor.SpriteIndex = 1;
...
DWORD InitializeSkeletor(void)
{
    ...
    gSkeletor.ScreenPosX = 192;

    gSkeletor.ScreenPosY = 64;

    gSkeletor.CurrentSuit = SUIT_0;

    gSkeletor.Direction = DOWN;



    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Down0_Standing.bmp", &gSkeletor.Sprite[SUIT_0][STANDING_DOWN_0])) != ERROR_SUCCESS)
    {
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Down1_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_DOWN_1])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Down2_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_DOWN_2])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Left0_Standing.bmp", &gSkeletor.Sprite[SUIT_0][STANDING_LEFT_0])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Left1_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_LEFT_1])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Left2_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_LEFT_2])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Right0_Standing.bmp", &gSkeletor.Sprite[SUIT_0][STANDING_RIGHT_0])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Right1_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_RIGHT_1])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Right2_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_RIGHT_2])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Up0_Standing.bmp", &gSkeletor.Sprite[SUIT_0][STANDING_UP_0])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Up1_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_UP_1])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
    ...
    if ((Error = Load32BppBitmapFromFile(".\\Assets\\Bitmaps\\SKELETOR\\Skeletor_Suit0_Up2_Walking.bmp", &gSkeletor.Sprite[SUIT_0][WALKING_UP_2])) != ERROR_SUCCESS)
    {
        MessageBoxA(NULL, "Load32BppBitmapFromFile failed!", "Error!", MB_ICONEXCLAMATION | MB_OK);
...
void RenderFrameGraphics(void)
{
    ...
    Blit32BppBitmapToBuffer(&gSkeletor.Sprite[gSkeletor.CurrentSuit][gSkeletor.Direction + gSkeletor.SpriteIndex],
        gSkeletor.ScreenPosX,
        gSkeletor.ScreenPosY);
...
if (Result != ERROR_SUCCESS)
{
    ...
    {
        LogMessageA(LL_ERROR, "[%s] Failed to read the 'LogLevel' value! Error 0x%08lx!", __FUNCTION__, Result);
...
{
    size_t MessageLenght = strlen(Message);
    ...
    if (MessageLenght < 1 || MessageLenght > 4095)
    {
    ...
    switch (LogLevel)
    {
        ...
        case LL_WARNING:
        {
            strcpy_s(SeverityString, sizeof(SeverityString), "[WARNING]");
...
void DrawDebugInfo(void)
{
    ...
    sprintf_s(DebugTextBuffer, sizeof(DebugTextBuffer), "FramesR:  %llu", gPerformanceData.TotalFramesRendered);

    BlitStringToBuffer(DebugTextBuffer, &g6x7Font, &White, 0, 64);

    sprintf_s(DebugTextBuffer, sizeof(DebugTextBuffer), "ScreenXY: (%d,%d)", gSkeletor.ScreenPosX, gSkeletor.ScreenPosY);
...
void FindFirstConnectedGamepad(void)
{
    ...
    for (int8_t GamepadIndex = 0; GamepadIndex < XUSER_MAX_COUNT && gGamepadID == -1; gGamepadID++)
    {
